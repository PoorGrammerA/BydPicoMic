        ...
        while (p_desc_end - p_desc > 0) {
          if (tu_desc_type(p_desc) == TUSB_DESC_ENDPOINT) {
            ...
            if (tud_audio_n_version(i) == 1) {
              // A no-sync UAC1 capture endpoint is still a data endpoint.
              // Only an OUT-side no-sync endpoint can be explicit feedback.
              audio10_desc_as_iso_data_ep_t const *desc_ep_uac1 = (audio10_desc_as_iso_data_ep_t const *) p_desc;
              bool const no_sync = desc_ep_uac1->bmAttributes.sync == TUSB_ISO_EP_ATT_NO_SYNC;
              bool const capture_in = tu_edpt_dir(desc_ep->bEndpointAddress) == TUSB_DIR_IN;
              is_data_ep = !no_sync || capture_in;
              is_feedback_ep = no_sync && !capture_in;
            } else {
